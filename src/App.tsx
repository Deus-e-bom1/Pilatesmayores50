...
function Cover({ onStart }: { onStart: () => void }) {
  return <div className="cover-content">
    ...
    <div className="age-grid">{" "}
      {["40-50 años", "51-60 años", "61 años o más"].map((age) => <Button key={age} onClick={onStart} className="age-button"><span className="option-circle" />{age}</Button>)}
    </div>
...
function Offer() {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => setCtaVisible(true), funnelConfig.ctaDelaySeconds * 1000);
    return () => window.clearTimeout(timer);
  }, []);
  ...
  return <div className="offer-wrap">
    ...
    <VturbPlayer />
    {funnelConfig.useOwnCta && ctaVisible && <Button className="offer-button" onClick={handleCheckout}><Sparkles />Quiero mi protocolo y mi acceso</Button>}
